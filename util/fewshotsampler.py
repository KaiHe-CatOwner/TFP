...
class FewshotSampler:
    ...
    def __next__(self):
        ...
        candidates_query = copy.copy(candidates)
        ...
        candidates_query = list(set(candidates_query) - set(support_idx))
        query_class = {'k':self.Q}
        query_idx = []
        repeat_count = 10
        while not self.__finish__(query_class):
            # print(query_class)
            index = random.choice(candidates_query)
            if index not in query_idx and index not in support_idx:
                ...
                candidates_query.remove(index)
            
            if len(candidates_query)==0:
                break
...
class SupportFewshotSampler(FewshotSampler):
    ...
    def __valid_sample__(self, sample, set_class, target_classes):
        ...
        for class_name in class_count:
            if class_name not in target_classes:
                # print("wrong class inlcude")
                return False
            
            if class_count[class_name] + set_class.get(class_name, 0) > threshold:
                # print("more than need", sample.class_count)
                return False
            
            if set_class.get(class_name, 0) <= set_class['k']:
                # print("ok sample", sample.class_count)
                isvalid = True
    ...
    def __next__(self):
        ...
        while not self.__finish__(support_class):
            ...
            if len(candidates)==0:
                ...
                repeat_count-=1
                if repeat_count==0:
                    # raise Exception("No enough data to sample !")
                    break
