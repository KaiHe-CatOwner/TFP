...
class FewShotNERModel(nn.Module):
    def __init__(self, sentence_encoder, ignore_index=-1):
        '''
    ...
    def forward(self, input_ids, lm_labels, tagging_labels, target_class):
        '''
    ...
    def __get_class_span_dict__(self, label, is_string=False):
        ...
        if not is_string:
            ...
            # having tags in string format ['none', 'none', 'xxx', ..]
            while i < len(label):
                if label[i] != 'none':
                    start = i
                    current_label = label[i]
    ...
    def __get_all_span__(self, label_span):
        '''
        return count of all entity spans
        '''
        label_span_list = []
        for label in label_span:
            label_span_list += label_span[label]
        return len(set(label_span_list))
    ...
    def __get_type_error__(self, pred, label, query):
        ...
        total_correct_span_num = self.__get_correct_span__(pred_span, label_span)  # num of correct entity spans
        total_span_num = self.__get_all_span__(label_span)              # num of correct entity spans
        
        wrong_within_span_num = self.__get_wrong_within_span__(pred_span, label_span)    # num of correct span, correct coarse type but wrong finegrained type
        wrong_outer_span_num = self.__get_wrong_outer_span__(pred_span, label_span)      # num of correct span but wrong coarse type
        return wrong_within_span_num, wrong_outer_span_num, total_span_num, total_correct_span_num
    ...
    def error_analysis(self, pred, label, query):
        ...
        pred = pred.cpu().numpy().tolist()
        label = label.cpu().numpy().tolist()
        within_num, outer_num, total_span_num, total_correct_span_num = self.__get_type_error__(pred, label, query)
        return fp, fn, len(pred), within_num, outer_num, total_span_num, total_correct_span_num
