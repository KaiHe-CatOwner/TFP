...
class ViterbiDecoder:
    ...
    def project_target_transitions(n_tag, abstract_transitions, tau):
        ...
        a = torch.eye(n_tag).cuda() * i_i
        # transitions from I-X to I-Y
        b = torch.ones(n_tag, n_tag).cuda() * x_y / (n_tag - 3)
        c = torch.eye(n_tag).cuda() * x_y / (n_tag - 3)
        transitions = a + b - c
        ...
        transitions = torch.where(transitions > torch.tensor(0).cuda(), transitions, torch.tensor(.000001).cuda())
